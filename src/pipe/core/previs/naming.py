"""Workspace-file naming for a previs sequence.

Pure string logic: derive the bare sequence letter, canonicalize a label, and
build the `<letter>_<label>_v###.mb` filename that both the manifest's FileRecord
and the Maya file ops use. Kept here so the naming rule lives in one place the
tests can cover without a DCC.
"""
